...
// TODO: Move to networking/pkg/
func ComputeHTTPRouteHash(hr *gwv1alpha1.HTTPRoute) ([sha256.Size]byte, error) {
	bytes, err := json.Marshal(hr.Spec)
	if err != nil {
		return [sha256.Size]byte{}, fmt.Errorf("failed to serialize Ingress: %w", err)
	}
	bytes = append(bytes, []byte(hr.GetNamespace())...)
	bytes = append(bytes, []byte(hr.GetName())...)
...
func TestProbeHeaders(t *testing.T) {
	...
	hr, client, _ := CreateHTTPRouteReady(ctx, t, clients, gwv1alpha1.HTTPRouteSpec{
	...
	bytes, err := ComputeHTTPRouteHash(hr)
	if err != nil {
