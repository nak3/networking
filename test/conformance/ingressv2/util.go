...
// OverrideHTTPRouteAnnotation overrides the Ingress annotation.
...
// CreateHTTPRoute creates a Knative Ingress resource
func CreateHTTPRoute(ctx context.Context, t *testing.T, clients *test.Clients, spec gatewayv1alpha1.HTTPRouteSpec, io ...Option) (*gatewayv1alpha1.HTTPRoute, context.CancelFunc) {
	...
	if err := reconciler.RetryTestErrors(func(attempts int) (err error) {
		...
		t.Fatalf("Error creating Ingress %q: %v", hrName, err)
	}

	return hr, func() {
		err := clients.GatewayAPIClient.HTTPRoutes.Delete(ctx, hr.Name, metav1.DeleteOptions{})
		if err != nil {
			t.Errorf("Error cleaning up Ingress %q: %v", hrName, err)
		}
	}
}
...
func UpdateIngressReady(ctx context.Context, t *testing.T, clients *test.Clients, name string, spec v1alpha1.IngressSpec) {
	...
	if err := WaitForHTTPRouteState(ctx, clients.GatewayAPIClient, name, IsHTTPRouteReady, t.Name()); err != nil {
...
func CreateDialContext(ctx context.Context, t *testing.T, clients *test.Clients) func(context.Context, string, string) (net.Conn, error) {
	...
	// TODO:
	internalDomain := "istio-ingressgateway.istio-system.svc"
